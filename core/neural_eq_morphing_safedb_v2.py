"""
Neural EQ Morphing System V2 - SAFE-DB Dataset
===============================================

FIXED VERSION with proper normalization and decoder constraints.

Key improvements from V1:
1. LOG-SCALE normalization for frequencies (critical!)
2. Proper decoder activations (tanh/softplus)
3. Balanced loss function (all params on same scale)
4. Hard clipping for safety
5. Contrastive weight annealing

Based on FlowEQ and audio ML best practices.
...
class SAFEDBDatasetLoaderV2:
    """
    IMPROVED Data Loader with proper normalization

    Key changes from V1:
    - LOG-SCALE for frequencies
    - Separate normalization per parameter type
    - Min-max normalization (not z-score)
    """
    ...
    def __init__(self, data_dir: str = "research/data"):
        ...
        # Normalization stats (computed during load)
        self.gain_min = -12.0
        self.gain_max = 12.0
        self.freq_log_min = None  # Will be computed
        self.freq_log_max = None
        self.q_log_min = None
        self.q_log_max = None

    def load_dataset(self, min_examples: int = 5) -> List[SAFEEQSetting]:
        """Load and preprocess SAFE-DB dataset with proper normalization"""

        print("="*70)
        print("LOADING SAFE-DB DATASET V2 (WITH LOG-SCALE NORMALIZATION)")
        print("="*70)
        ...
        # Compute normalization stats from ALL filtered data
        print(f"\nComputing normalization statistics...")
        self._compute_normalization_stats(raw_settings)
    ...
    def _compute_normalization_stats(self, settings: List[SAFEEQSetting]):
        """Compute min/max for normalization"""

        all_params = np.array([s.eq_params for s in settings])

        # Frequency indices: 1, 3, 6, 9, 12
        freq_indices = [1, 3, 6, 9, 12]
        all_freqs = all_params[:, freq_indices]

        # Q indices: 4, 7, 10
        q_indices = [4, 7, 10]
        all_qs = all_params[:, q_indices]

        # Log-scale frequencies
        freqs_log = np.log10(all_freqs + 1)  # +1 to avoid log(0)
        self.freq_log_min = freqs_log.min()
        self.freq_log_max = freqs_log.max()

        # Log-scale Q
        qs_log = np.log10(all_qs)
        self.q_log_min = qs_log.min()
        self.q_log_max = qs_log.max()
...
class NeuralEQEncoderV2(nn.Module):
    """Encoder V2 (same as V1, normalization fixed upstream)"""
...
class NeuralEQDecoderV2(nn.Module):
    """
    IMPROVED Decoder V2
...
class NeuralEQMorphingSAFEDBV2:
    """
    ...
    def __init__(self, latent_dim: int = 32, device: str = None):
        ...
        # Models
        self.encoder = NeuralEQEncoderV2(latent_dim=latent_dim).to(self.device)
        self.decoder = NeuralEQDecoderV2(latent_dim=latent_dim).to(self.device)

        # Data
        self.loader = SAFEDBDatasetLoaderV2()
        self.eq_settings = []
        self.term_to_idx = {}
        self.idx_to_term = {}
